...
CMD_FLAGS = '--chat --model-menu --model anon8231489123_vicuna-13b-GPTQ-4bit-128g  --no-stream --api --gpu-memory 12 --cpu-memory 5GB --verbose --settings settings.json --auto-devices'
...
def run_model():
    os.chdir("text-generation-webui")
    run_cmd(f"python server.py {CMD_FLAGS}  --wbits 4 --groupsize 128 --model_type llama", environment=True)
